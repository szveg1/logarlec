...
public class SzobaFelosztTest {
    public void test() {
        ...
        if (labirintus.getSzobak().get(0).getEmberek().size() - labirintus.getSzobak().get(1).getEmberek().size() <= 1) Szkeleton.logger.info("Letrejott szobaba az emberek fele kerult");
        else Szkeleton.logger.log(Level.WARNING, "Letrejott szobaba baj van az emberek szamaval");

        if (labirintus.getSzobak().get(0).getItems().size() - labirintus.getSzobak().get(1).getItems().size() <= 1) Szkeleton.logger.info("Letrejott szobába a targyak fele kerult");
